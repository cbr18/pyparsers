...
// ReplaceBySource — атомарно заменяет данные по источнику (удаление + вставка) в одной транзакции
func (r *CarPostgres) ReplaceBySource(ctx context.Context, source string, cars []domain.Car) error {
    tx, err := r.db.BeginTx(ctx, nil)
    if err != nil {
        return err
    }
    ...
    if _, err := tx.ExecContext(ctx, "DELETE FROM cars WHERE source = $1", source); err != nil {
        return err
    }
