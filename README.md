...
Проект использует централизованную систему переменных окружения. Подробная документация находится в файле [ENVIRONMENT_SETUP.md](ENVIRONMENT_SETUP.md).
...
2. Отредактируйте переменные под ваши нужды
3. Перезапустите приложения

Основные переменные:
- `API_HOST` - хост API сервера
- `API_PORT` - порт API сервера
- `CORS_ALLOW_ORIGINS` - разрешенные источники для CORS
